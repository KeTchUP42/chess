...
import chess.figures.src.Abstract.AbstractFigure;
...
public class Pawn extends AbstractFigure implements IObject {
    ...
    private boolean pawnStepValid(int SquareNumber, IArea Board) {
        ...
        byte multiplier = (byte) (this.color == Color.WHITE ? 1 : -1);
        ...
        return ((SquareNumber == this.getSquareNumber() + step * 2) && this.getSquareNumber() == this.startPosition) ||
                (attackValid && SquareNumber == ordStep + 1 && Board.getXCoordinate(ordStep + 1) == Board.getXCoordinate(this.squareNumber) + 1) ||
                (attackValid && SquareNumber == ordStep - 1 && Board.getXCoordinate(ordStep - 1) == Board.getXCoordinate(this.squareNumber) - 1) ||
                (SquareNumber == ordStep);
