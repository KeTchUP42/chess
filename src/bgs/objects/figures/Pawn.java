...
public class Pawn extends AbstractFigure {
    ...
    private boolean pawnStepValid(int SquareNumber, @NotNull IArea Board) {
        ...
        int ordStep = this.squareNumber + step;
        //Attack valid check
        boolean attackValid = Board.getObjectFromList(SquareNumber) != null && Board.getObjectFromList(SquareNumber).getColor() != this.color;
        //Empty square check
        boolean nullSquare = Board.getObjectFromList(SquareNumber) == null;

        return ((SquareNumber == this.squareNumber + step * 2) && this.squareNumber == this.startPosition && nullSquare && this.isWayFreePerpendicular(SquareNumber, Board)) ||
